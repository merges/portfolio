...
## provisioning-bionic.sh 1.0
## Spencer Wohlers

## This script takes a fresh Ubuntu Server 18.04 "Bionic Beaver" install and
## configures it for use as a server for The Visual, Jeff's portfolio website.

## Differences over Artful Aardvark:
##   - no more external apache2 dependency
...
apt-get install build-essential openssh-server linux-headers-generic npm node-gyp software-properties-common apache2 -y >>$OUTPUT_LOG 2>>$ERROR_LOG
...
echo "( 3/$TOTAL_STEPS) Stopping Apache, if running."
systemctl stop apache2 >>$OUTPUT_LOG 2>>$ERROR_LOG

# Add user to group
echo "( 4/$TOTAL_STEPS) Adding user 'jeff' to group 'www-data'."
...
echo "( 8/$TOTAL_STEPS) Enabling Apache dependencies."
...
echo "(10/$TOTAL_STEPS) Creating Diffie-Hellman group for Forward Secrecy. (Be patient.)"
openssl dhparam -out /etc/ssl/certs/dhparam.pem 2048 >>$OUTPUT_LOG 2>>/dev/null

# Setup Apache configuration
echo "(11/$TOTAL_STEPS) Configuring Apache."
...
echo "(14/$TOTAL_STEPS) Starting Apache."
